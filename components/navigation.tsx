...
import { NAVIGATION_ITEMS, EMSCULPT_SUBMENU_ITEMS, EMFACE_SUBMENU_ITEMS, ABOUT_SUBMENU_ITEMS } from "@/lib/constants"
    ...
    <nav className="fixed top-10 w-screen bg-background bg-opacity-80 backdrop-blur-md z-40">
      <Container className="h-16 flex items-center justify-between">
              ...
              <NavigationMenuItem key={item.href}>
                {item.labelKey === "nav_emsculpt_neo" ? (
                  ...
                  <>
                    ...
                    <NavigationMenuContent className="md:left-0 align-left-dropdown" data-align="left">
                      <div className="grid w-[400px] gap-3 p-4">
                  ...
                  <>
                    ...
                    <NavigationMenuContent>
                      <div className="grid w-[300px] gap-3 p-4">
                        {ABOUT_SUBMENU_ITEMS.map((subItem) => (
                          <Link 
                            key={subItem.href}
                            href={'external' in subItem ? subItem.href : createLocalizedHref(subItem.href, locale)}
                            className="block select-none space-y-1 rounded-md p-3 leading-none no-underline outline-none transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground"
                            {...('external' in subItem && { target: "_blank", rel: "noopener noreferrer" })}
                          >
                            <div className="text-sm font-semibold leading-none">
                              {t(subItem.titleKey)}
                            </div>
        ...
        <div className="hidden md:flex items-center gap-2">
          <Button asChild size="sm" className="font-semibold group px-4">
